...
describe('release automation', () => {
  ...
  it('uses changesets release PRs and publishes merged versions from the changelog', () => {
    ...
    expect(workflow).toContain('- beta')
    expect(workflow).toContain("github.ref_name == 'beta'")
    expect(workflow).toContain('Release channel beta must run from beta branch')
    expect(workflow).toContain('Release channel latest must run from main branch')
    expect(workflow).toContain('changesets/action@v1')
    expect(workflow).toContain("version: pnpm version:release")
    expect(workflow).toContain('pnpm run version:prerelease:enter:beta')
    expect(workflow).toContain("steps.changesets.outputs.hasChangesets == 'false'")
    expect(workflow).toContain('pnpm run ci')
    expect(workflow).toContain('PUBLISH_SCRIPT=release:publish')
    expect(workflow).toContain('PUBLISH_SCRIPT=release:publish:beta')
    expect(workflow).toContain('pnpm run ${{ steps.release-config.outputs.publish_script }}')
    expect(workflow).toContain('npm view "$PACKAGE_NAME" version')
    expect(workflow).toContain('npm view "$PACKAGE_NAME@beta" version')
    ...
    expect(workflow).toContain('git add -A package.json CHANGELOG.md .changeset')
    expect(workflow).toContain('git commit -m "chore: release ${PACKAGE_VERSION} [skip release]"')
    expect(workflow).toContain('git push origin "HEAD:${GITHUB_REF_NAME}"')
    expect(workflow).toContain('pnpm run release:notes')
    expect(workflow).toContain('GITHUB_RELEASE_FLAGS=--prerelease')
  ...
  it('documents beta releases as a long-lived prerelease branch workflow', () => {
    const changesetReadme = readFileSync(join(root, '.changeset', 'README.md'), 'utf8')

    expect(changesetReadme).toContain('beta 分支')
    expect(changesetReadme).toContain('push 到 `beta`')
    expect(changesetReadme).toContain('自动发布')
    expect(changesetReadme).toContain('不要长期把 prerelease 状态留在 `main`')
  })
})
