...
describe('rewrite session automation', () => {
  ...
  it('parses completed and pending families from the inventory doc', async () => {
    ...
    expect(mod.parsePendingFamilies(inventoryDoc)).toContain('position / inset leftovers / float / z / order / box-sizing')
    expect(mod.findNextPendingFamily(inventoryDoc)).toBe('position / inset leftovers / float / z / order / box-sizing')
