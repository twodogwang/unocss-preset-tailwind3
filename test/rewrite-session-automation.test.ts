...
describe('rewrite session automation', () => {
  ...
  it('parses completed and pending families from the inventory doc', async () => {
    ...
    expect(mod.parseCompletedFamilies(inventoryDoc)).toContain('white-space / breaks / hyphens / content-visibility / contents / field-sizing / color-scheme')
    expect(mod.parseCompletedFamilies(inventoryDoc)).toContain('fill')
    expect(mod.parsePendingFamilies(inventoryDoc)).toEqual([])
    expect(mod.findNextPendingFamily(inventoryDoc)).toBeUndefined()
