...
describe('rewrite session automation', () => {
  ...
  it('parses completed and pending families from the inventory doc', async () => {
    ...
    expect(mod.parsePendingFamilies(inventoryDoc)).toContain('size / width / height / min / max')
    expect(mod.findNextPendingFamily(inventoryDoc)).toBe('size / width / height / min / max')
