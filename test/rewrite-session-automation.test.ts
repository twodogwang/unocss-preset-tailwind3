...
describe('rewrite session automation', () => {
  ...
  it('parses completed and pending families from the inventory doc', async () => {
    ...
    expect(mod.parsePendingFamilies(inventoryDoc)).toContain('cursor / pointer-events / resize / user-select')
    expect(mod.findNextPendingFamily(inventoryDoc)).toBe('cursor / pointer-events / resize / user-select')
