...
describe('rewrite session automation', () => {
  ...
  it('parses completed and pending families from the inventory doc', async () => {
    ...
    expect(mod.parsePendingFamilies(inventoryDoc)).toContain('text-wrap / text-overflow / text-transform')
    expect(mod.findNextPendingFamily(inventoryDoc)).toBe('text-wrap / text-overflow / text-transform')
