...
把 `accent` 主规则族从当前 `svg / accent / caret colors` 混合测试段中拆出，纳入已经稳定的 source rewrite 模板：shared fixture、runtime、Tailwind parity、utility spec、blocklist migration 子集，以及独立的过程文档与总表同步。
...
- `accent-[#fff]`
- 主题扩展场景 `accent-brand`
- `accent` 相关高置信度 blocklist migration
...
- `accent-(.+)` 通过 color resolver 处理颜色
- runtime 与 parity 目前主要通过 `svg / accent / caret colors` 混合测试约束
- blocklist 中已有 `accent-#fff -> accent-[#fff]`，但尚未形成 `accent` 专用共享 fixture
...
这一轮只建立 `accent` 专用模板，不顺手把 `caret` 一并并入。

### 2. 没有真实偏差就不改 runtime

`src/_rules-wind3/behaviors.ts` 当前实现已经与 Tailwind 3 `accent` 语义一致。  
...
`accent-red500`、`accent-opacity-50` 与 `accent-op50` 不提供 replacement，只保留为非法样例。
...
- [test/fixtures/tailwind-accent-rewrite.ts](/Users/shakugannoshana/Documents/ex/twodogwang/unocss-preset-tailwind3/test/fixtures/tailwind-accent-rewrite.ts)
...
- [docs/2026-04-23-accent-source-rewrite-log.md](/Users/shakugannoshana/Documents/ex/twodogwang/unocss-preset-tailwind3/docs/2026-04-23-accent-source-rewrite-log.md)
- [docs/2026-04-23-accent-source-rewrite-status.md](/Users/shakugannoshana/Documents/ex/twodogwang/unocss-preset-tailwind3/docs/2026-04-23-accent-source-rewrite-status.md)
