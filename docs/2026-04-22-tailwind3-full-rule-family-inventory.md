...
| `flex` | `src/_rules/flex.ts`, `src/_theme/default.ts`, `src/_theme/misc.ts`, `src/_theme/size.ts`, `src/_theme/types.ts` | `completed_template` | 已对齐 Tailwind 3.4 的 `flex` / `grow` / `shrink` / `basis` 语义，补齐默认 `flexBasis/flexGrow/flexShrink` key，并仅保留 `flex-inline` / `flex-basis-*` 高置信度旧别名治理；`flex-grow` / `flex-shrink` 长写法按 Tailwind 官方语义支持 |
...
| `scroll-behavior` | `src/_rules-wind3/behaviors.ts` | `completed_template` | 已对齐 Tailwind 3.4 的 scroll-behavior 官方静态语义，并移除 `scroll-*` global keyword shortcut |
