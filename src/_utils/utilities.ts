...
export function resolveTailwindSpacing(theme: Theme, size: string | undefined, options: { allowAuto?: boolean, allowFraction?: boolean } = {}) {
  ...
  const themed = theme.spacing?.[size]
  if (themed != null)
    return themed
...
export function directionThemeSize(propertyPrefix: string, options: { allowAuto?: boolean } = {}): DynamicMatcher {
  return ([_, direction, size]: string[], { theme }: RuleContext<Theme>): CSSEntries | undefined => {
    const v = resolveTailwindSpacing(theme, size, options)
    if (v != null)
      return directionMap[direction].map(i => [`${propertyPrefix}${i}`, v])
  }
}
