...
import { colorableShadows, colorResolver, h, isCSSMathFn, splitShorthand } from '../utils'
...
export const fonts: Rule<Theme>[] = [
  ...
  // opacity
  [/^text-opacity-(.+)$/, ([, opacity]) => ({ '--un-text-opacity': h.bracket.percent.cssvar(opacity) }), { autocomplete: 'text-opacity-<percent>' }],
  ...
  [
    /^leading-(.+)$/,
    ([, s], { theme }) => {
      const value = theme.lineHeight?.[s] ?? (/^\d+(?:\.\d+)?$/.test(s) ? h.bracket.cssvar.global.rem(s) : undefined) ?? (s.startsWith('[') ? h.bracket.cssvar.global.rem(s) : undefined)
      if (value != null)
